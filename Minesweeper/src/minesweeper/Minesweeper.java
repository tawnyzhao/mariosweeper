 ...
 * 
 *          Extra Features: Double clicking Custom cursor Detailed score
 *          tracking
 */
...
public class Minesweeper {
    ...
    private ArrayList<Integer> generateMines(ArrayList<Integer> notPermissible) {
        ...
        for (int i = 0; i < NUM_MINES; i++) {
            int newMine = rand.nextInt(rows * cols);
            if (mineList.contains(newMine) || notPermissible.contains(newMine)) { // Only add mines to list if it isn't
        ...
        for (int i = 0; i < grid.length; i++) {
            for (int j = 0; j < grid[i].length; j++) {
                if (grid[i][j] == -1) { // If loops through a mine, increment the surrounding tiles
                    for (int a = -1; a <= 1; a++) {
                        for (int b = -1; b <= 1; b++) {
                            if (i + a >= 0 && i + a < grid.length && j + b >= 0 && j + b < grid[i].length) { // Checks
                                                                                                             // if tiles
                                                                                                             // is not
                                                                                                             // out of
                                                                                                             // bounds
    ...
    /**
     * Builds the grid with the value of the button, ensuring that there is no mines
     * generated adjacent to the give coordinate
     * 
    ...
    void exposeEmpty(int[] coord) {
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                if (coord[0] + i >= 0 && coord[0] + i < rows && coord[1] + j >= 0 && coord[1] + j < cols) {
                    if (!buttons[coord[0] + i][coord[1] + j].isExposed()
                            && !buttons[coord[0] + i][coord[1] + j].isFlagged()) {
    ...
    void clearNear(int[] coord) {
        ...
        if (flagCount == buttons[x][y].getValue()) { // Checks if the correct amount of flags are around the tile before
                                                     // clearing
            for (int i = -1; i <= 1; i++) {
                for (int j = -1; j <= 1; j++) {
                    if (x + i >= 0 && x + i < rows && y + j >= 0 && y + j < cols) {
                        if (!buttons[x + i][y + j].isExposed()) {
                            if (!buttons[x + i][y + j].isFlagged() && buttons[x + i][y + j].getValue() == -1) { // Ends
                                                                                                                // game
                                                                                                                // if
                                                                                                                // incorrectly
                                                                                                                // flagged
     ...
     * 
     * @return ImageIcon
     * @param url an image file to convert
     */
    private ImageIcon generateIcon(URL url) {
        return new ImageIcon(new ImageIcon(url).getImage().getScaledInstance(tileSize, tileSize, Image.SCALE_SMOOTH));
    }
    ...
    void initializeImages() {
        TILEIMG = generateIcon(Minesweeper.class.getResource("tile.png"));
        FLAGIMG = generateIcon(Minesweeper.class.getResource("flag.png"));
        BOMBIMG = generateIcon(Minesweeper.class.getResource("mine.png"));
        ACTIVEIMG = generateIcon(Minesweeper.class.getResource("active.png"));
        RESTARTIMG = generateIcon(Minesweeper.class.getResource("restart.png"));
        CURSORIMG = generateIcon(Minesweeper.class.getResource("cursor.png"));
        FLOWERIMG = generateIcon(Minesweeper.class.getResource("flower.png"));
    }
    ...
    void initializeSounds() {
        sounds = new Sound[] {
            new Sound(Minesweeper.class.getResource("athletic.wav")),
            new Sound(Minesweeper.class.getResource("flowergarden.wav")),
            new Sound(Minesweeper.class.getResource("fever.wav")),
            new Sound(Minesweeper.class.getResource("lullaby.wav")),
            new Sound(Minesweeper.class.getResource("underwater.wav")),
            new Sound(Minesweeper.class.getResource("overworld.wav"))
            };
    }
    
    public Minesweeper(){
        // Setting constants
        scoreHandler = new HighscoreHandler(Minesweeper.class.getResource("highscores.txt").getFile());
